...
export class EvmTransactionComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    this.subs.push(this.activatedRoute.params.subscribe(async (routeParams) => {
      ...
      const [hash, receipt, trace] = await Promise.all([
        this.evm.getTransactionByHash(this.txHash),
        this.evm.getTransactionReceipt(this.txHash),
        this.evm.traceTransaction(this.txHash)
      ]);
